...
export default function EmeritusMembersPage() {
  const { query, boardOnly, statusFilter } = useMembersFilters();
  const [emeritusMembers, setEmeritusMembers] = useState<Member[]>([]);
  ...
  useEffect(() => {
    const loadMembers = async () => {
      try {
        const res = await fetch('/api/members/emeritus', {
          cache: 'no-store'
        });
        ...
        setEmeritusMembers(data.emeritus ?? []);
      } catch (error) {
  ...
  console.log('Emeritus Members:', emeritusMembers);

  const filtered = useMemo(() => {
    const term = query.trim().toLowerCase();

    return emeritusMembers
  ...
  }, [boardOnly, emeritusMembers, query, statusFilter]);
